...
int variacionSinRepeticion(int n, int m) {
	int num = n;
	int den = n - m;
...
void resuelveCaso() {
	...
	while (eq1 != "FIN") {
		...
		if (!m.contains(g)) m[g] = 0;
		if (!m.contains(p)) m[p] = 0;
	...
	for (auto const& cv : m) {
		...
		else if (cv.valor == max) ++mC;
	}
	if (mC == 1) std::cout << gan << " " << m.size()*(m.size() - 1) - cont << '\n';
	else std::cout << "EMPATE " << m.size()*(m.size() - 1) - cont << '\n';
}
