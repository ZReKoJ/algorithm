...
template <typename T>
bintree<T> leerArbol(T vacio, int &i, const std::string &palabra) {
	T raiz;
	raiz = palabra.at(i);
	if (raiz == vacio) { // es un árbol vacío
		...
		++i;
		auto iz = leerArbol(vacio, i, palabra);
		++i;
		auto dr = leerArbol(vacio, i, palabra);
		return{ iz, raiz, dr };
	}
}
...
bool resuelveCaso() {
	...
	if (!std::cin.fail()) {
		int j = 0;
		bintree<char> t = leerArbol(' ', j, palabra);
		std::vector<char> v = t.postorder();
