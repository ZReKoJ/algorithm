...
void numeracion(int pruebas) {
	...
	for (int i = 0; i < pruebas; ++i) {
		...
		int a = 0;
		if (tipo == "CORRECTO") ++a;
		else if (tipo == "INCORRECTO") --a;
		if (m.contains(nombre)) m[nombre] += a;
		else m[nombre] = a;
	}
	for (auto const& cv : m) {
		if (cv.valor != 0) printf("%s, %d\n", cv.clave.c_str(), cv.valor);
	}
	printf("---\n");
}
