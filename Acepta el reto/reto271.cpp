...
bool resuelveCaso() {
	int medio, numValores;
	std::cin >> medio >> numValores;
	if (medio == 0 && numValores == 0) return false;
	else {
		std::vector<int> v;
		PriorityQueue<int, std::greater<int>> qMayor;
		PriorityQueue<int, std::less<int>> qMenor;
		int num;
		for (int i = 0; i < numValores; ++i) {
			...
			if (qMayor.size() > qMenor.size()) {
				qMenor.push(medio);
				medio = qMayor.top();
				qMayor.pop();
			}
			else if (qMayor.size() < qMenor.size()) {
				qMayor.push(medio);
				medio = qMenor.top();
				qMenor.pop();
			}
			v.emplace_back(medio);
		}
		std::cout << v[0];
...
O(2n(log n))
	n es el numero de parejas de aves insertadas
*/
#define DOMJUDGE
int main() {
#ifndef DOMJUDGE
