...
template <class T>
tDatos conteo(bintree<T> const& arbol) {
	if (arbol.empty()) {
		return{ 0, 0, 0 };
	}
		...
		tDatos izq, dch;
		izq = conteo(arbol.left());
		dch = conteo(arbol.right());
		return{ std::max(izq.alt, dch.alt) + 1, izq.nodo + dch.nodo + 1, izq.hoja + dch.hoja };
	}
}
...
void resuelveCaso() {
	...
	tDatos datos = conteo(t);
	std::cout << datos.nodo << " " << datos.hoja << " " << datos.alt << '\n';
}
