...
template <class T>
T buscarMin(const bintree<T> &t) {
	auto min = t.root();
	if (!t.left().empty()) {
		auto izq = buscarMin(t.left());
		if (izq < min) min = izq;
	}
	if (!t.right().empty()) {
		auto dch = buscarMin(t.right());
		if (dch < min) min = dch;
	}
	return min;
}
...
bool resuelveCaso() {
	...
	if (!std::cin.fail()) {
		if (letra == 'N') {
			int vacio = -1;
			bintree<int> t = leerArbol(vacio);
			std::cout << buscarMin(t) << "\n";
		}
		else {
			std::string vacio = "#";
			bintree<std::string> t = leerArbol(vacio);
			std::cout << buscarMin(t) << "\n";
		}
		return true;
	}
