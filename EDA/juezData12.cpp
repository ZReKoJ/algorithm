...
template <class T>
bool rescate(const bintree<T> &arbol, int suma, int &max, int &num) {
	if (!arbol.empty()) {
		suma += arbol.root();
		if (suma > max)
			max = suma;
		bool izq = rescate(arbol.left(), suma, max, num);
		bool dch = rescate(arbol.right(), suma, max, num);
		if (!izq && !dch && arbol.root() != 0)
			++num;
		return izq || dch || arbol.root() != 0;
	}
	else return false;
}
