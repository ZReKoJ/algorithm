...
bool resuelveCaso() {
	...
	if (numLineas != 0) {
		...
		map<std::string, std::vector<bool>> m;

		for (int i = 0; i < numLineas; ++i) {
			getline(std::cin, texto);
			for (int i = 0; i < texto.size(); ++i) if (isupper(texto.at(i))) texto.at(i) = tolower(texto.at(i));
			v = split(texto, ' ');
			for (std::string s : v) {
				if (s.size() > 2) {
					if (!m.contains(s)) {
						for (int i = 0; i < numLineas; ++i)
							m[s].emplace_back(false);
					}
					m[s][i] = true;
				}
			}
