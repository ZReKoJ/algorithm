...
template <class T>
int conteo(bintree<T> const& arbol, int &numGen, bool &c) {
	if (arbol.empty()) {
		numGen = 0;
		return -2;
	}
		...
		int numGeni, numGend;
		int izq = conteo(arbol.left(), numGeni, c);
		int dch = conteo(arbol.right(), numGend, c);
		if (arbol.root() - 18 >= izq && izq - 2 >= dch) {
			numGen = std::max(numGeni, numGend) + 1;
		}
		else if (izq == -2 && dch == -2) {
			numGen = std::max(numGeni, numGend) + 1;
		}
