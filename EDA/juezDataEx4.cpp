...
std::pair<int, int> completo(bintree<char> const& t) {
	if (t.empty())
		return{ 0, 0 };
	else {
		auto izq = completo(t.left());
		auto dch = completo(t.right());
		std::pair <int, int> p = { std::max(izq.first, dch.first), std::min(izq.second, dch.second) };
		++p.second;
