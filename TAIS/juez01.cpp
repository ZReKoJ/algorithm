...
std::pair<int, bool> resolver(bintree<char> const& t) {
	if (t.empty()) return{ 0, true };
	else {
		auto izq = resolver(t.left());
		auto dch = resolver(t.right());
		return{ std::max(izq.first, dch.first) + 1, izq.second && dch.second && abs(izq.first - dch.first) <= 1 };
	}
}

void resuelveCaso() {
	bintree<char> t = leerArbol('.');
	if (resolver(t).second) std::cout << "SI\n";
	else std::cout << "NO\n";
}
