...
tResultado resolver(bintree<int> const& t) {
	if (t.empty()) return{ true, 0, 0, INT_MAX };
	else {
		auto izq = resolver(t.left());
		auto dch = resolver(t.right());
		return{ izq.equilibrado && dch.equilibrado && abs(izq.altura - dch.altura) <= 1 && izq.max < t.root() && t.root() < dch.min,
			std::max(izq.altura, dch.altura) + 1,
			std::max(dch.max, t.root()),
			std::min(izq.min, t.root())
		};
	}
}
...
void resuelveCaso() {
	...
	if (resolver(t).equilibrado) std::cout << "SI\n";
	else std::cout << "NO\n";
}
