...
public:
	...
	void printRange(Clave const& inf, Clave const& sup) const {
		printRange2(inf, sup, raiz);
	}
...
protected:
	...
	void printRange2(Clave const& inf, Clave const&sup, Link a) const {
		if (a != nullptr) {
			int n = a->cv.clave;
			if (menor(inf, n))
				printRange2(inf, sup, a->iz);
			if (!menor(n, inf) && !menor(sup, n))
				std::cout << n << " ";
			if (menor(n, sup))
				printRange2(inf, sup, a->dr);
		}
	}
...
void resolver(map<int, int> const& t, int const& inf, int const& sup) {
	t.printRange(inf, sup);
	std::cout << "\n";
}
