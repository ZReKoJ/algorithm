...
public:
	...
	int buscarPos(int const& pos) {
		return buscarPos2(pos, raiz);
	}
...
protected:
	...
	int buscarPos2(int const& pos, Link a) {
		if (a == nullptr) throw std::domain_error("No existe");
		else {
			if (a->tam_i < pos) return buscarPos2(pos - a->tam_i, a->dr);
			else if (pos < a->tam_i) return buscarPos2(pos, a->iz);
			else return a->cv.clave;
		}
	}
	...
	bool inserta(clave_valor const& cv, Link & a) {
		...
		if (a == nullptr) {
			...
			insertado = inserta(cv, a->iz);
			if (insertado)
				++a->tam_i;
			reequilibraDer(a);
		}
		else if (menor(a->cv.clave, cv.clave)) {
			insertado = inserta(cv, a->dr);
			reequilibraIzq(a);
		}
...
bool resuelveCaso() {
	...
	if (numValores != 0) {
		...
		for (int i = 0; i < numValores; ++i) {
			std::cin >> number;
			t.insert({ number, number });
		}
		std::cin >> numValores;
		for (int i = 0; i < numValores; ++i) {
			std::cin >> number;
			try {	std::cout << t.buscarPos(number) << "\n";	}
			catch (std::domain_error &err) {	std::cout << "??\n";	}
		}
		std::cout << "----\n";
		return true;
	}
