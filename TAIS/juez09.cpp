...
int dfs(Grafo const& G, size_t v, std::vector<bool> & marked) {
	int n = 1;
	marked[v] = true; // para no repetir vértices
	for (auto w : G.adj(v)) { // recorremos todos los adyacentes a v
		if (!marked[w]) {
			n += dfs(G, w, marked);
		}
...
void resuelveCaso() {
	int pers, parAmigos, v, w;
	std::cin >> pers >> parAmigos;
	Grafo g(pers);
	for (int i = 0; i < parAmigos; ++i) {
		std::cin >> v >> w;
		g.ponArista(v - 1, w - 1);
	}
	resolver(g);
...
O(n + m)
	n es el numero de amigos
	m es el numero de pares de personas que se conoce que son amigos
*/
int main() {
#ifndef DOMJUDGE
