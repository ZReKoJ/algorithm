...
bool dfs(Grafo const& G, size_t v, std::vector<std::pair<bool, int>> & marked) {
	bool valid = true;
	marked[v].first = true; // para no repetir vértices
	int n = 1;
	if (marked[v].second == 1) n = -1;
	for (auto w : G.adj(v)) { // recorremos todos los adyacentes a v
		if (!marked[w].first) {
			marked[w].second = n;
			valid = valid && dfs(G, w, marked);
		}
		else {
			if (marked[w].second != n) valid = false;
		}
	}
	return valid;
}
...
void resolver(Grafo const& g) {
	std::vector<std::pair<bool, int>> marked(g.V(), { false, 0 });
	bool valid = true;
	for (int i = 0; i < g.V(); ++i)
		if (!marked[i].first) {
			marked[i].second = 1;
			valid = valid && dfs(g, i, marked);
		}
 	if (valid) std::cout << "SI\n";
	else std::cout << "NO\n";
}
